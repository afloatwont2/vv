...
clear all;
close all;
[signal, fs] = rdsamp('s0010_re', 1, 500);  % Read 500 samples from the signal
...
%xlim([0 2]);  % Adjust the x-axis as needed

%% FIR Low-Pass Filter (Remove High-Frequency Noise)
...
%% IIR Notch Filter (Remove Powerline Interference)
...
xlabel('Time (s)'); ylabel('Amplitude');
title('IIR Notch Filtered ECG Signal (Remove 50 Hz)');
%xlim([0 2]);
