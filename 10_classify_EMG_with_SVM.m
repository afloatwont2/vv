...
[emg1, fs1, t1] = rdsamp('session3_participant1_gesture10_trial1');  % replace with actual filename
[emg2, fs2, t2] = rdsamp('session3_participant1_gesture11_trial1');  % second gesture
...
for i = 1:step_size:(length(emg1_filt) - window_size)
    ...
    mav = mean(abs(segment));           % Mean Absolute Value (MAV)
    rms_val = rms(segment);             % Root Mean Square (RMS)
    zc = sum(diff(sign(segment)) ~= 0); % Zero Crossing (ZC)
